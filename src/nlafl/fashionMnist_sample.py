...
def load_fashionMNIST():
    """ Load the FASHION MNIST dataet

    Returns:
        tuple: tuple of numpy arrays trn_x, trn_y, tst_x, tst_y
    """

    trn_x = np.load(os.path.expanduser(common.fashionMnist_trn_x_pth))
    trn_y = np.load(os.path.expanduser(common.fashionMnist_trn_y_pth))
    tst_x = np.load(os.path.expanduser(common.fashionMnist_tst_x_pth))
    tst_y = np.load(os.path.expanduser(common.fashionMnist_tst_y_pth))
...
def partition(x, y):
    ...
    all_x = []
    y_list = range(common.num_classes['fashionMnist'])
...
def dirichlet_sample(all_x, num_clients, client_size, alpha=100, seed=None):
    ...
    num_classes = common.num_classes['fashionMnist']
    clients = []
...
if __name__ == '__main__':
    ...
    if poison_ct > 0:
        clients = fixed_poison(
            ...
            client_targ=cli_targ, targ_frac=targ_frac, alpha=1, seed=seed
        )
