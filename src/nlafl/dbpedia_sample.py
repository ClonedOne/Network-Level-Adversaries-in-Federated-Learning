...
def load_dbpedia():
    """ Load the dbpedia dataet

    Returns:
        tuple: tuple of numpy arrays trn_x, trn_y, tst_x, tst_y
    """

    trn_x = np.load(os.path.expanduser(common.dbpedia_trn_x_pth))
    trn_y = np.load(os.path.expanduser(common.dbpedia_trn_y_pth))
    tst_x = np.load(os.path.expanduser(common.dbpedia_tst_x_pth))
    tst_y = np.load(os.path.expanduser(common.dbpedia_tst_y_pth))
...
def partition(x, y):
    ...
    all_x = []
    y_list = range(common.num_classes['dbpedia'])
...
def dirichlet_sample(all_x, num_clients, client_size, alpha=100, seed=None):
    ...
    num_classes = common.num_classes['dbpedia']
    clients = []
