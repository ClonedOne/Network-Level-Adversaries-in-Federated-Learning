...
from nlafl import dbpedia_sample
from nlafl.dbpedia_models import FLServer
from nlafl.dbpedia_models import FederatedClient
...
def update_information(
    ...
    cur_performance_defender = new_model.evaluate(
        tst_x_defender, tst_y_defender, verbose=0)
    cur_performance_attacker = new_model.evaluate(
    ...
    agg_perf_diff_defender = cur_performance_defender[0] - \
        last_performance_defender
    ...
    agg_perf_diff_attacker = cur_performance_attacker[0] - \
        last_performance_attacker
    if normalize_performance:
...
def main():
    ...
    # generate clients
    trn_x, trn_y, tst_x, tst_y = dbpedia_sample.load_dbpedia()
    partitioned = dbpedia_sample.partition(trn_x, trn_y)
    tst_partitioned = dbpedia_sample.partition(tst_x, tst_y)
    ...
    if args.poison_count > 0:
        client_data = dbpedia_sample.fixed_poison(
            partitioned,
            NUM_USERS,
            CLIENT_SIZE,
            args.poison_count,
        ...
        client_data = dbpedia_sample.fixed_sample(
            partitioned,
            NUM_USERS,
            CLIENT_SIZE,
            targ_class=args.target_class,
    ...
    for r in range(NUM_ROUNDS):
        ...
        current_round_argmax_sample_probs = np.argsort(sample_probs)[
            :NUM_CLIENTS]

        # Store selection probs argmax of current round
        per_round_selection_probs_argmax.append(
            current_round_argmax_sample_probs)

        # Store current round target_clients
        per_round_target_users.append(deepcopy(target_clients))

        # Calculate adversary overlap
        adversary_overlap = len(
            [i for i in target_clients if i in pop_clients])

        # Calculate defender overlap
        defender_overlap = len(
            [i for i in current_round_argmax_sample_probs if i in pop_clients])
        ...
        for client_ind in idxs_users:
            ...
            if (args.num_pop_clients <= client_ind) and (client_ind < args.poison_count + args.num_pop_clients):
                ...
                print('Performance of the current poisoned client on the target data {}'.format(
                    client.local_model.model.evaluate(
                        pop_ds_attacker[0], pop_ds_attacker[1], verbose=0)
                ))
        ...
        per_round_agg_improvements_attacker.append(
            deepcopy(agg_improvements_attacker))
        per_round_agg_improvements_defender.append(
            deepcopy(agg_improvements_defender))
        per_round_each_improvements_attacker.append(
            deepcopy(each_improvements_attacker))
        per_round_each_improvements_defender.append(
            deepcopy(each_improvements_defender))
